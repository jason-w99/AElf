...
public class KernelTestHelper
{
    ...
    public List<Transaction> GenerateTransactions(int count, long refBlockNumber = 0, Hash refBlockHash = null)
    {
        ...
        for (var i = 0; i < count; i++) transactions.Add(GenerateTransaction(refBlockNumber, refBlockHash));
        ...
        LogEvent logEvent = null)
    {
        ...
        if (logEvent != null) transactionResult.Logs.Add(logEvent);
        ...
        List<Transaction> transactions = null, Dictionary<string, ByteString> extraData = null)
    {
        ...
        {
            foreach (var transaction in transactions) newBlock.AddTransaction(transaction);
    ...
    {
        if (transactions == null || transactions.Count == 0) transactions = new List<Transaction>();

        if (transactions.Count == 0) transactions.Add(GenerateTransaction());

        if (transactionResults == null) transactionResults = new List<TransactionResult>();
